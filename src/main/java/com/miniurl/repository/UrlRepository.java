...
public interface UrlRepository extends JpaRepository<Url, Long> {
     ...
     */
    @Query("SELECT COUNT(u) FROM Url u WHERE u.user.id = :userId AND YEAR(u.createdAt) = :year AND MONTH(u.createdAt) = :month")
     ...
     */
    @Query("SELECT COUNT(u) FROM Url u WHERE u.user.id = :userId AND DATE(u.createdAt) = :day")
