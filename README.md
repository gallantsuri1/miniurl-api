...
- **Features Table**: Master feature definitions (9 features)
- **Feature Flags Table**: Role-based enabled status (16 flags - 8 per role)
- **Global Flags Table**: Features not tied to roles (1 flag - USER_SIGNUP)

ADMIN users can manage features for both USER and ADMIN roles independently.

📖 **Complete Documentation**: See [`scripts/FEATURE_FLAGS_GUIDE.md`](scripts/FEATURE_FLAGS_GUIDE.md) for the complete API reference, examples, and troubleshooting guide.

### Quick Reference

**Features (9 total):**
1. `USER_SIGNUP` - User Sign Up (GLOBAL)
...
- Global Flags: 1 row (USER_SIGNUP)

### API Endpoints

📖 **Complete API Documentation**: See [`scripts/FEATURE_FLAGS_GUIDE.md`](scripts/FEATURE_FLAGS_GUIDE.md) for detailed request/response examples.
...
miniurl/
├── scripts/
│   ├── pre-push                 # Git hook: blocks direct pushes to main
...
- **Direct pushes blocked** via git hook
...
This installs a `pre-push` hook that prevents direct pushes to `main`/`master`.
